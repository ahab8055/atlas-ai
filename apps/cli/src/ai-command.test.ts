...
describe("ai CLI command", () => {
  it("prints help for bare ai", async () => {
    ...
    try {
      ...
      expect(writes.join("")).toContain("atlas ai providers");
    } finally {
  ...
  it("quantization recommends levels for hardware", async () => {
    const writes: string[] = [];
    const originalWrite = process.stdout.write.bind(process.stdout);
    process.stdout.write = ((chunk: string | Uint8Array) => {
      writes.push(String(chunk));
    ...
    try {
      const handled = await tryHandleAiCommand("ai quantization");
      expect(handled).toBe(true);
      const text = writes.join("");
